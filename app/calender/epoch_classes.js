...
function Epoch(name,mode,targetelement,multiselect,tag_jo)
{
	this.state = 0;
	this.name = name;
...
Epoch.prototype.calConfig = function () //PRIVATE: initialize calendar variables
{
	...
	this.rangeYearUpper = 2025;
	this.minDate = new Date(2005,0,1);
	this.maxDate = new Date(2025,0,1);
...
CalCell.prototype.onclick = function () 
{
	...
	if(!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
	{
		...
		else //if we can only select one cell at a time
		{
			...
			{
				//tagged_jo = this.tag_jo ;
				tagged_jo = "-" ;
				owner.tgt.value = owner.outputDate(owner.selectedDates[0],'yyyy' + tagged_jo + 'mm' + tagged_jo + 'dd');
				if(owner.mode == 'popup') {
					owner.hide();
				}
			}
